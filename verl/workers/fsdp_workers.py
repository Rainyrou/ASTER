...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # Full params: only when actor is initialized and FSDP v1
        actor_fsdp_is_v1 = (
            hasattr(self, "actor_module_fsdp")
            and self.actor_module_fsdp is not None
    ...
    def init_model_for_validation(self):
        ...
        # override model kwargs (only load config, not weights)
        actor_model_config = AutoConfig.from_pretrained(
            local_path, trust_remote_code=trust_remote_code, attn_implementation="flash_attention_2"
        )
        ...
        # Apply override_config to config (for vLLM/sglang context length validation, etc.)
        ...
        # Save to instance for rollout use; and set skip weight sync flag
        self.actor_model_config = actor_model_config
        self._skip_weight_sync = True

        # Only build rollout, engine loads weights from disk; do not create optimizer / scheduler / DP actor
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_hf_only_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        """Save only HuggingFace format checkpoint to save disk space."""
        import os
        ...
        # Only support actor
        assert self._is_actor, "HF-only checkpoint only supports actor role"
        # Not supporting LoRA for HF-only export now
        assert not self._is_lora, "LoRA HF-only export is not supported yet, please disable LoRA and try again"
        ...
        # Rank0 collects full state dict in CPU for saving
        state_dict = get_fsdp_full_state_dict(self.actor_module_fsdp, offload_to_cpu=True, rank0_only=True)

        if dist.get_rank() == 0:
            # Unwrap FSDP to read config
            if fsdp_version(self.actor_module_fsdp) == 1:
                unwrap_model = self.actor_module_fsdp._fsdp_wrapped_module
            else:
                unwrap_model = self.actor_module_fsdp
            ...
            if hasattr(unwrap_model, "can_generate") and unwrap_model.can_generate() and hasattr(
            ...
            # Save model config and tokenizer/processor
            model_config.save_pretrained(hf_local_path)
            processing_class = self.processor if self.processor is not None else self.tokenizer
            processing_class.save_pretrained(hf_local_path)

            # Determine correct AutoModel class
            ...
            # Create empty model and save with state dict
            with init_empty_weights():
                save_model = auto_model_cls.from_config(model_config, torch_dtype=torch.bfloat16)
            save_model.to_empty(device="cpu")
            if hasattr(save_model, "can_generate") and save_model.can_generate():
                if generation_config is not None:
                    save_model.generation_config = generation_config
            save_model.save_pretrained(hf_local_path, state_dict=state_dict)

            log_with_rank(
                f"Saved HF-only checkpoint to {os.path.abspath(hf_local_path)}",
                rank=dist.get_rank(),
                logger=logger,
                log_only_rank_0=True,
            )
