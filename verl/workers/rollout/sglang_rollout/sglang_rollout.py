...
def get_tool_call_parser_type(
    ...
    for parser_type, parser_cls in items:
        ...
        if parser.bot_token.strip() in tokenizer_vocab and (
            ...
            return parser_type
        if parser_type in processing_class.name_or_path:
            return parser_type
    else:
...
class SGLangRollout(BaseRollout):
    ...
    async def _handle_engine_generate(
        ...
        whole_max_new_tokens = sampling_params.get("max_new_tokens", self.config.response_length)
        max_new_tokens = min(whole_max_new_tokens, self.config.max_model_len - len(generation_prompt_ids) - 1)
