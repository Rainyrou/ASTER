...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            if wandb_key:
                print("wandb login with key: ", wandb_key)
                wandb.login(key=wandb_key)

            settings = None            
            if settings is None and (os.environ.get("WANDB_INSECURE_DISABLE_SSL") == "true" or 
                                     os.environ.get("WANDB_VERIFY_SSL") == "false"):
...
class TrainingGenerationsLogger:
    """Logger for training generation samples to various backends"""
    
    def log(self, loggers, samples, step):
        """Log training generation samples to all configured loggers
        
        Args:
            loggers: Dictionary of logger instances from Tracking class
            samples: List of (input, output, score) tuples
    ...
    def log_generations_to_swanlab(self, samples, step):
        ...
        # Log to swanlab
        swanlab.log({"train/generations": swanlab_table}, step=step)

    def log_generations_to_wandb(self, samples, step):
        """Log training generation samples to WandB"""
        import wandb

        # Create or update training generations table
        columns = ["step", "input", "output", "score"]
        
        if not hasattr(self, "training_table"):
            self.training_table = wandb.Table(columns=columns)
        
        # Create new table with existing data plus new samples
        new_table = wandb.Table(columns=columns, data=self.training_table.data)
        
        # Add new samples
        for sample in samples:
            new_table.add_data(step, sample[0], sample[1], sample[2])
    ...
    def log_generations_to_vemlp_wandb(self, samples, step):
        """Log training generation samples to Volcengine ML Platform WandB"""
        from volcengine_ml_platform import wandb as vemlp_wandb

        columns = ["step", "input", "output", "score"]
        
        if not hasattr(self, "vemlp_training_table"):
            self.vemlp_training_table = vemlp_wandb.Table(columns=columns)
        
        new_table = vemlp_wandb.Table(columns=columns, data=self.vemlp_training_table.data)
        
        for sample in samples:
            new_table.add_data(step, sample[0], sample[1], sample[2])
        
        vemlp_wandb.log({"train/generations": new_table}, step=step)
        self.vemlp_training_table = new_table

    def log_generations_to_mlflow(self, samples, step):
        """Log training generation samples to MLflow as artifacts"""
        import json
        import tempfile
        import mlflow
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                training_gen_step_file = Path(tmp_dir, f"train_step{step}.json")
                row_data = [
                    {"input": sample[0], "output": sample[1], "score": sample[2]}
                    for sample in samples
                ]
                with open(training_gen_step_file, "w") as file:
                    json.dump(row_data, file)
                mlflow.log_artifact(training_gen_step_file)
        except Exception as e:
            print(f"WARNING: save training generation file to mlflow failed with error {e}")

    def log_generations_to_clearml(self, samples, step):
        """Log training generation samples to ClearML as table"""
        import clearml
        import pandas as pd
    ...
    def log_generations_to_tensorboard(self, samples, step):
        """Log training generation samples to TensorBoard as text"""
        if not hasattr(self, "writer"):
            from torch.utils.tensorboard import SummaryWriter
        ...
        text_content = f"**Training Generation Results - Step {step}**\n\n"

        for i, sample in enumerate(samples):
            text_content += f"### Sample {i + 1}\n"
            text_content += f"**Input:** {sample[0]}\n\n"
            text_content += f"**Output:** {sample[1]}\n\n"
            text_content += f"**Score:** {sample[2]}\n\n"
            text_content += "---\n\n"

        self.writer.add_text("train/generations", text_content, step)
        self.writer.flush()
