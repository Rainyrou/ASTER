...
def process_validation_metrics(
    ...
    Returns:
        ...
        - "pass@K": Estimated pass@K (probability at least one correct in K draws) if a boolean
          correctness signal exists ("is_correct"/"correct") or if numeric "score" is binary 0/1.
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if isinstance(var_vals[0], (str, type(None))):
                    continue
            ...
            # After processing all variables for this uid, compute pass@K if correctness available
            correct_vals = None
            if "is_correct" in var2vals:
                correct_vals = [bool(v) for v in var2vals["is_correct"]]
            elif "correct" in var2vals:
                correct_vals = [bool(v) for v in var2vals["correct"]]
            elif "score" in var2vals:
                # Treat strictly binary scores as correctness
                score_vals = var2vals["score"]
                if all(isinstance(v, (int, float)) for v in score_vals) and set(score_vals).issubset({0, 1, 0.0, 1.0}):
                    correct_vals = [bool(v) for v in score_vals]

            if correct_vals is not None:
                n_resps = len(correct_vals)
                c = int(np.sum(correct_vals))
                pass_metric = {}
                if n_resps > 0:
                    ns = []
                    n = 2
                    ...
                    for k in ns:
                        if k <= n_resps:
                            if c == 0:
                                ...
                                num = comb(n_resps - c, k) if (n_resps - c) >= k else 0
                                den = comb(n_resps, k)
                                pass_k = 1.0 - (num / den)
                            pass_metric[f"pass@{k}"] = float(pass_k)

                data_src2uid2var2metric[data_source][uid]["pass"] = pass_metric
