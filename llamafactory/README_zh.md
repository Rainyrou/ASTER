...
✅ **已支持**：llamafactory现在支持通过 `drop_exceeding_samples` 参数来控制超过指定长度的样本是否参与训练。设置 `drop_exceeding_samples=True` 时，超过 `cutoff_len` 的样本将被丢弃而不是截断。截断的逻辑在：
![](./assets/cutoff.png)


原始数据集在文件夹：
data/original_oss_data/gpt-oss-20b-reasoning，merged.jsonl为其他两个数据集的merge版本。数量为45K。
```bash
wc -l data/original_oss_data/gpt-oss-20b-reasoning/merged.jsonl 
45622 data/original_oss_data/gpt-oss-20b-reasoning/merged.jsonl
```

转换脚本：
```bash
python scripts/convert_qwen3_thinking_tools.py --input data/original_oss_data/gpt-oss-20b-reasoning/merged.jsonl  --output data/qwen3_tool_cold_start.parquet
```
