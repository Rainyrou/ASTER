...
A secure code sandbox environment for running and evaluating code generated by large language models, supporting multiple programming languages and **stateful Python code execution**.

> **📢 New Feature**: This repository is forked from [ByteDance/SandboxFusion](https://github.com/bytedance/SandboxFusion) and adds support for **stateful Python code execution**, allowing sequential execution of code snippets within the same session.
...
python3.11 -m venv sandbox-runtime
source sandbox-runtime/bin/activate
pip install uv
export UV_CACHE_DIR=$PWD/.uv_cache
uv pip install -r ../requirements.txt
```
...
source env/sandbox/bin/activate
mkdir -p docs/build

# Start service (runs in background)
make run-online > run.log 2>&1 &
```

### Testing Stateful Python Code Execution

3. **Test the New Feature**

```bash
# Execute first code snippet in session
curl -X POST http://127.0.0.1:8080/run_stateful_code \
  -H "Content-Type: application/json" \
  -d '{"code": "a = 1; b = 2", "language": "python", "session_id": "test"}'

# Execute second code snippet in same session (can access previously defined variables)
curl -X POST http://127.0.0.1:8080/run_stateful_code \
  -H "Content-Type: application/json" \
  -d '{"code": "print(f\"Result: {a + b}\")", "language": "python", "session_id": "test"}'
```

### Advanced Configuration

Service configuration can be adjusted by modifying the Makefile:
...
# Run command
run-online:
	- pkill -f "sandbox_clean.py" || true
	- bash -c 'pids=$$(lsof -ti:$(PORT) -sTCP:LISTEN 2>/dev/null || true); if [ -n "$$pids" ]; then kill -9 $$pids || true; fi'
	nohup python3 sandbox_clean.py > sandbox_clean.log 2>&1 &
	uvicorn sandbox.server.server:app --host 127.0.0.1 --port $(PORT) --workers $(THREAD_NUM) --loop uvloop --http httptools
```

## 📖 Usage Examples

### Stateful Python Code Execution

Unlike traditional code sandboxes, this version supports sequential execution of code snippets within the same session, where variables and function definitions persist across executions:
...
# Second execution - use previously defined content
code_2 = """
print(f"x = {x}")
print(f"fibonacci({x}) = {y}")
z = [fibonacci(i) for i in range(5)]
print(f"First 5 fibonacci numbers: {z}")
"""
```
...
```bash
# Create session and define data
curl -X POST http://127.0.0.1:8080/run_stateful_code \
  -H "Content-Type: application/json" \
  -d '{
...
# Perform data analysis in the same session
curl -X POST http://127.0.0.1:8080/run_stateful_code \
  -H "Content-Type: application/json" \
  -d '{
    "code": "print(df.describe()); result = df.sum().to_dict()",
    "language": "python",
    "session_id": "data_analysis"
  }'
```
