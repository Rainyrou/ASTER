...
def get_python_rt_env(env_name: str):
    # fix: using a pre-built image virtual environment
    sandbox_root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    activate_file_path = os.path.join(sandbox_root_dir, 'env', env_name, 'bin', 'activate')
    r = subprocess.run(f'bash -c "source {activate_file_path} && which python"',
                       capture_output=True,
                       text=True,
                       check=True,
                       shell=True)
...
async def get_cpp_rt_flags():
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        # fix: encoding='utf-8'
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.cpp', delete=False, encoding='utf-8') as f:
...
async def run_python(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.py', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_go_test(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='_test.go', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_go(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        for file in os.listdir(source_dir):
            shutil.copy2(os.path.join(source_dir, file), tmp_dir)
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.go', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_nodejs(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.js', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_typescript(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        os.symlink(os.path.join(deps_dir, 'node_modules'), os.path.join(tmp_dir, 'node_modules'))
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.ts', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_jest(args: CodeRunArgs) -> CodeRunResult:
    ...
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        ...
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.test.ts', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_php(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.php', delete=False, encoding='utf-8') as f:
...
async def run_rust(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.rs', delete=False, encoding='utf-8') as f:
            f.write(args.code)
...
async def run_bash(args: CodeRunArgs) -> CodeRunResult:
    with tempfile.TemporaryDirectory(dir=get_tmp_dir(), ignore_cleanup_errors=True) as tmp_dir:
        restore_files(tmp_dir, args.files)
        with tempfile.NamedTemporaryFile(mode='w', dir=tmp_dir, suffix='.sh', delete=False, encoding='utf-8') as f:
            f.write(args.code)
