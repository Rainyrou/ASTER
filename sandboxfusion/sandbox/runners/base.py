...
async def run_command_bare(command: str | List[str],
    ...
    try:
        ...
        try:
            ...
            # Strong cleanup: try to kill the whole process group first, then the tree as fallback
            try:
...
async def run_commands(compile_command: Optional[str], run_command: str, cwd: str, extra_env: Optional[Dict[str, str]],
    ...
    if config.sandbox.isolation == 'none':
        ...
        # Apply CPU time limit to guarantee hard-stop by kernel
        def cpu_time_limit_preexec():
            try:
                soft, hard = resource.getrlimit(resource.RLIMIT_CPU)
                cpu_limit_seconds = max(1, int(args.run_timeout))
                # Do not exceed hard limit
                limit = cpu_limit_seconds if hard == resource.RLIM_INFINITY else min(cpu_limit_seconds, hard)
                resource.setrlimit(resource.RLIMIT_CPU, (limit, hard))
            except Exception:
                # Best-effort; ignore on platforms not supporting RLIMIT_CPU
                pass
        preexec_steps.insert(0, cpu_time_limit_preexec)

        # Make the child the leader of a new session/process group, so we can kill the whole group
        preexec_steps.insert(0, os.setsid)
        ...
        async with tmp_overlayfs() as root, tmp_cgroup(mem_limit='4G', cpu_limit=1) as cgroups, tmp_netns(
            ...
            # Start a new session for the command inside the namespace as well
            preexec_fn = os.setsid

            if compile_command is not None:
                compile_res = await run_command_bare(prefix + ['bash', '-c', f'cd {cwd} && {compile_command}'],
                                                     args.compile_timeout, None, cwd, extra_env, True, preexec_fn=preexec_fn)
            if compile_res is None or (compile_res.status == CommandRunStatus.Finished and
                ...
                run_res = await run_command_bare(prefix + ['bash', '-c', f'cd {cwd} && {run_command}'],
                                                 args.run_timeout, args.stdin, cwd, extra_env, True, preexec_fn=preexec_fn)
